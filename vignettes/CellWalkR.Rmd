...
corresponding peaks into a GRanges object. **Please note:** all data needed to
recreate this vignette locally should already loaded so read commands can be skipped.
They are included as an example for how new data could be loaded.
...
mapping. However this will require installing packages for genomic annotaions 
(GenomicFeatures and a TxDb object). We'll use the Hg38 in this example.

```{r installAnnotations, eval=FALSE}
...
for each other. Plotting requires the packages ggplot2 and reshape2. 
