...
load the corresponding peaks into a GRanges object. **Please note:** all
data needed to recreate this vignette locally should already loaded so
read commands can be skipped. They are included as an example for how
new data could be loaded. If working with a SnapATAC object, a
cell-by-peak matrix should be stored in @pmat.
...
gene bodies. Note that getRegions requires GenomicFeatures and a TxDb
